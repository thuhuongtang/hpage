...
<%
	...
	//request.setAttribute("headq000", Text.getText(resource, "headq000", null, locale));
	for(int i=0; i<=5; i++) {
...
<template:tmpt title="${menu004}">
	<jsp:attribute name="sub_menu">
		<H2>${menu000}</H2>
		<UL class=s_dep02>
			<LI>
				<A href="javascript:void(0)" onclick="navigate('./overview.do','hpage_contents-service','OV')">${menu001}</A>
			</LI>
			...
			<LI class=on>
				<A class=indep3 href="javascript:void(0)" onclick="navigate('./headquarters.do','hpage_contents-service','HQ')">${menu004}</A></LI>	
			<LI>
				<A href="javascript:void(0)" onclick="navigate('./ictvietnam.do','hpage_contents-service','IV')">${menu005}</A></LI>				
		</UL>
	</jsp:attribute>
	...
	<DIV class="con_head biz_ref">
		...
		<DIV class=hg>
			<H3>${menu004}</H3>
			<P>${menu004}</P>
			<IMG style="overflow: auto; float: right; margin-top: -150px;" src="img/hq.jpg">
		</DIV>
		<!-- con_head -->
		<!-- con_body -->
