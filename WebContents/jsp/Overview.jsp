...
<%
	...
	request.setAttribute("oview000", Text.getText(resource, "oview000", null, locale));
	for(int i=0; i<=5; i++) {
...
<template:tmpt title="${menu001 }">
	<jsp:attribute name="sub_menu">
		<H2>${menu000 }</H2>
		<UL class=s_dep02>
			<LI class=on>
				<A class=indep3	href="javascript:void(0)" onclick="navigate('./overview.do','hpage_contents-service','OV')">
					${menu001 }
				</A>
			</LI>
			<LI>
				<A href="javascript:void(0)" onclick="navigate('./message.do','hpage_contents-service','GD')">
					${menu002 }
				</A>
			</LI>
			<LI>
	...
	<DIV class="con_head biz_ref">
		...
		<DIV class=hg>
			<H3>${menu001 }</H3>
			<P>${oview000 }</P>
			<IMG alt="posco ict vietnam" style="overflow: auto; float: right;margin-top: -150px;" src="img/stock-app-icon.png">
		</DIV>
		<!-- con_head -->
		<!-- con_body -->
