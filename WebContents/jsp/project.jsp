...
<%
	...
	if(ctx != null) {
		...
		PosRowSet rsResult1 = ctx.getRowSet("RS_RESULT1");
		if(rsResult1 != null && rsResult1.count() > 0) {
			PosRow row1 = rsResult1.next();
			HPageContent contDtl = new HPageContent(); 
			contDtl.setSubtitle(row1.getAttribute("sub_title").toString());
			contDtl.setSubtitle1(row1.getAttribute("sub_title_1").toString());
			contDtl.setSubtitle2(row1.getAttribute("sub_title_2").toString());
			contDtl.setContentDetails(row1.getAttribute("content_details").toString());
			request.setAttribute("contDtl", contDtl);
		}
	}	
...
<template:tmpt title="${menu009}">
	<jsp:attribute name="sub_menu">
		...
		<UL class=s_dep02>
			<LI>
				<A href="javascript:void(0)" onclick="navigate('./itsbrt.do','hpage_contents-service','OV')">${menu006}</A>
			</LI>
			<LI>
	...
	<DIV class="con_head biz_ref">
		...
		<!-- con_head -->
		<DIV class=hg>
			<H3>${contDtl.subtitle1}</H3>
		</DIV>
		<!-- con_head -->
		<!-- con_body -->
		<DIV class="con_body ci">
			${contDtl.contentDetails}
		</DIV>
	</DIV>
	</jsp:attribute>
</template:tmpt>