...
<%
	...
	request.setAttribute("ictvn000", Text.getText(resource, "ictvn000", null, locale));
	for(int i=0; i<=5; i++) {
...
<template:tmpt title="${menu005}">
	<jsp:attribute name="sub_menu">
		<H2>${menu000}</H2>
		<UL class=s_dep02>
			<LI><A href="javascript:void(0)" onclick="navigate('./overview.do','hpage_contents-service','OV')">${menu001}</A></LI>
			<LI><A href="javascript:void(0)" onclick="navigate('./message.do','hpage_contents-service','GD')">${menu002}</A></LI>
			<LI><A href="javascript:void(0)" onclick="navigate('./pglobal.do','hpage_contents-service','GP')">${menu003}</A></LI>
			<LI><A href="javascript:void(0)" onclick="navigate('./headquarters.do','hpage_contents-service','HQ')">${menu004}</A></LI>	
			<LI class=on><A class=indep3 href="javascript:void(0)" onclick="navigate('./ictvietnam.do','hpage_contents-service','IV')">${menu005}</A></LI>				
		</UL>
	</jsp:attribute>
	<jsp:attribute name="content">
	<DIV class="con_head biz_ref">
		<DIV class=bcs>
					<SPAN class=home><SPAN class=hide>home</SPAN></SPAN> &gt; ${menu000} &gt; ${menu005}
				</DIV>
				<!-- con_head -->
				<DIV class=hg>
					<H3>${menu005}</H3>
					<P>${ictvn000 }</P>
					<IMG style="overflow: auto; float: right;margin-top: -150px;" src="img/gold-winner.jpg">
				</DIV>
				<!-- con_head -->
				<!-- con_body -->
